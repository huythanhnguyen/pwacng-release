...
import defaultClasses from "./uploadImageField.module.scss";
    ...
    return (
        <label className={absolute ? `${classes.root} ${classes.rootAbsolute}` : classes.root}>
            <input
                ...
                key={imageResetKey}
            />
            <span className={classes.inputTrigger} title={formatMessage({ id: 'uploadImage.trigger', defaultMessage: 'Upload Image' })}><span>Upload Image</span></span>
        </label>
    );
}
