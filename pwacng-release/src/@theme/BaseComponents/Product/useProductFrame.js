 ...
 */
export const useProductFrame = props => {
    const { mapProduct, pathname } = props;

    const operations = mergeOperations(DEFAULT_OPERATIONS, props.operations);
    const { data: resolveUrlData } = useQuery(RESOLVE_URL, {
    ...
    const slug = (resolveUrlData?.route?.relative_url || pathname).split('/').pop();
