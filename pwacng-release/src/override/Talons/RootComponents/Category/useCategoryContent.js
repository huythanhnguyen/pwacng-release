...
export const useCategoryContent = props => {
    ...
    const filters = filterData?.products?.aggregations || null;
    const items = data?.products?.items || placeholderItems;
    const totalPagesFromData = data?.products?.page_info?.total_pages || null;
    const totalCount = data?.products?.total_count || null;
    const categoryName = categoryData?.categories?.items?.[0]?.name || null;
    const categoryDescription = categoryData?.categories?.items?.[0]?.description || null;
    const categoryImage = categoryData?.categories?.items?.[0]?.category_menu_background || null;
    const availableSortMethods = sortData?.products?.sort_fields?.options || null;
