...
export const usePaymentInformation = props => {
    ...
    const [setBillingAddress, {called}] = useMutation(setBillingAddressMutation);
    ...
    const selectedPaymentMethod = paymentInformationData?.cart?.selected_payment_method?.code || null;
    ...
    const shippingAddressOnCart = paymentInformationData?.cart?.shipping_addresses?.[0] || null;
    ...
    useEffect(() => {
        if (shippingAddressOnCart && !called) {
            const {
                firstname = '',
                street = [],
                city = '',
    ...
    useEffect(() => {
        if (doneEditing && selectedPaymentMethod && cartId) {
            dispatch({
