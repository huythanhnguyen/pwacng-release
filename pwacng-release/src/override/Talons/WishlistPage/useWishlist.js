...
import defaultOperations from '../../Talons/WishlistPage/wishlist.gql';
...
export const useWishlist = (props = {}) => {
    const { id, itemsCount, setWishlistSelected } = props;
    ...
    useEffect(() => {
        setPage(1);
        if (itemsCount >= 1 && !data) {
            fetchWishlistItems();
        }
    }, [itemsCount, fetchWishlistItems, data]);
