...
const getMediaGalleryEntries = (product, optionCodes, optionSelections) => {
    ...
    return (product.small_image?.url && !value.length) ? [{
        "uid": product.uid,
        "file": product.small_image.url,
        "label": product.ecom_name || product.name,
        "position": 1,
        "disabled": false
    }] : value;
};
...
export const useProductFullDetail = props => {
    ...
    const breadcrumbCategoryId = useMemo(() => {
        if (product?.main_category?.uid) {
            return product.main_category.uid
        } else {
            return getBreadcrumbCategoryId(product.categories)
        }
    }, [product]);

    const mainCategoryName = useMemo(() => {
        return product?.main_category?.name || null;
    }, [product]);
            ...
            if (hasDeprecatedOperationProp) {
                ...
                const variables = {
                    ...
                    entered_options: [
                        {
                            uid: product.uid,
                            value: product?.ecom_name || product.name
                        }
                    ]
                };
