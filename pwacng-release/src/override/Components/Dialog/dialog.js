...
import dialogClasses from '@magenest/theme/BaseComponents/Dialog/extendStyle/dialog.module.scss';
...
const Dialog = props => {
    const {
        ...
        shouldBorderlessStyle = false,
        shouldShowButtons = true,
        shouldShowCloseXButton = true,
        shouldShowMask = true,
        shouldUnmountOnHide,
        title,
    ...
    const classes = useStyle(defaultClasses, dialogClasses, props.classes);
    const rootClass = [
        isOpen ? classes.root_open : classes.root,
        zIndex ? classes.overlapZIndex : '',
        (shouldBorderlessStyle || customClass) ? classes.dialogBorderlessWrapper : '',
    ...
    const maybeCloseXButton = (!isModal && shouldShowCloseXButton) ? (
        <button
    ...
    const maybeForm =
        isOpen || !shouldUnmountOnHide ? (
            ...
            >
                {/* The Mask. */}
                <button
                    className={shouldShowMask ? classes.mask : classes.maskTransparent}
                    disabled={isMaskDisabled}
                    onClick={onCancel}
                    type="reset"
                />
                {/* The Dialog. */}
                <div className={`${classes.dialog} ${shouldBorderlessStyle ? classes.dialogBorderless : ''} ${customClass ? classes.customInner : ''}`} data-cy={title}>
...
Dialog.propTypes = {
    ...
    classes: shape({
        body: string,
        dialogBorderlessWrapper: string,
        dialogBorderless: string,
        cancelButton: string,
